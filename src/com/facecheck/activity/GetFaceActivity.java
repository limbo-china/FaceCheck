...
public class GetFaceActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		userId = StringDefine.getCachedUserId(this);
		password = StringDefine.getCachedPassword(this);
...
//		});
		...
		btn_identify.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				if(img == null) {
					Toast.makeText(GetFaceActivity.this, "请先拍照", Toast.LENGTH_SHORT).show();
					return;
				}
				if(tag_is_two){
					Toast.makeText(GetFaceActivity.this, "该照片重复，请重新拍照", Toast.LENGTH_SHORT).show();
					return;
				}
				//cacheBitmap(bitmap);
				...
				final ProgressDialog pd = ProgressDialog.show(GetFaceActivity.this,"连接中","连接服务器中,请稍候");
				...
				faceppDetect.setDetectCallback(new DetectCallback() {
					...
					public void detectResult(final JSONObject rst) {
						...
						try {
							...
							img = bitmap;
							GetFaceActivity.this.runOnUiThread(new Runnable() {
								public void run() {
									pd.dismiss();
									if(faceCount == 0){
										Toast.makeText(GetFaceActivity.this, "未识别到人脸，可能是光线问题，请重新拍照", Toast.LENGTH_SHORT).show();
									}else if(faceCount != 1){
										Toast.makeText(GetFaceActivity.this, "识别到多张人脸，请重新拍照", Toast.LENGTH_SHORT).show();
									}else{
										try {
										...
										Toast.makeText(GetFaceActivity.this, "识别成功", Toast.LENGTH_SHORT).show();
									}
					...
					public void run() {
						...
						try {
							...
							try {
								...
								System.out.println(StringDefine.getCachedUserClass(GetFaceActivity.this)+"-->"
									+StringDefine.getCachedUserName(GetFaceActivity.this));
								if(!tag_is_person){  //还未建立person
									httpRequests.personCreate(new PostParameters().setPersonName("person_"+userId).
											setTag(StringDefine.getCachedUserClass(GetFaceActivity.this)+":"
									+StringDefine.getCachedUserName(GetFaceActivity.this)));
								...
								if(face_id==null){
									GetFaceActivity.this.runOnUiThread(new Runnable() {
										public void run() {
											Toast.makeText(GetFaceActivity.this,
